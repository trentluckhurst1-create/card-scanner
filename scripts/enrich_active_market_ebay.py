...
def enrich_payload(payload: dict, *, max_targets: int = 24, results_per_target: int = 50) -> dict:
    ...
    metrics.update({"ebay_enabled": not bool(missing), "ebay_targets": 0, "ebay_queries": 0, "ebay_added_cards": 0, "ebay_errors": 0})
    if missing:
    ...
    for sport, player, year in targets:
        ...
        for row in found:
            ...
            if identity is None or _text(identity.player) != _text(player) or str(identity.year or "").strip() != year:
                continue
