...
def _shared_player_year_targets(
    listings: Iterable[Listing], *, max_targets: int
    ...
    candidates = [
        (sport, labels[(sport, player, year)], year, sources)
        for (sport, player, year), sources in grouped.items()
        if len(sources) >= 2
    ]
...
def expand_shared_player_inventory(
    ...
    *,
    ...
    stores_by_token = {
        _source_token(getattr(store.source, "name", None) or store.name): store
        ...
        for source_token in sorted(participant_tokens):
            ...
            for row in found:
                ...
                if (
                    ...
                    and _text_token(identity.player) == target_player
                    and str(identity.year or "").strip() == year
                ):
                    expanded.append(row)
...
def _dashboard_record_with_aud(
    listing: Listing,
    fx_provider: RbaFxProvider,
    *,
    rate_date: date,
) -> dict:
    record = market_listing_to_dashboard_record(
        MarketListingObservation(listing=listing)
    )
    currency = str(listing.currency or "").upper()
    total = float(listing.price) + float(listing.shipping or 0.0)
    if currency == "AUD":
...
def build_active_market_payload(
    ...
    *,
    ...
    rate_date = rate_date or datetime.now(timezone.utc).date()
    cards = [
        _dashboard_record_with_aud(row, fx_provider, rate_date=rate_date)
        for row in rows
    ]
    ...
    metrics = {
        ...
        "foreign_currency_cards": sum(1 for card in cards if card.get("currency") != "AUD"),
        "fx_converted_cards": sum(
...
def cloud_active_store_source() -> MultiStoreSource:
    ...
    raw_stores = [
        *source.stores,
        ...
        NamedStoreSource(name="CLFox Collectables", source=CLFoxSource()),
        NamedStoreSource(name="Local Card Shop", source=LocalCardShopSource()),
        NamedStoreSource(
            name="House of Cards N Collectables",
            source=HouseOfCardsSource(),
        ),
    ]
    ...
    *,
    ...
    for sport in SPORTS:
        result = source.collect(
            sport=sport,
            query="",
            limit=limit_per_store_per_sport,
        )
    ...
    return (
        _dedupe(listings),
        stores_considered_total,
        max(stores_searched_total, len(searched_store_names)),
        errors,
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Publish a secret-free active-store comparison feed."
    )
    ...
    print(f"FOREIGN_CURRENCY_CARDS={metrics['foreign_currency_cards']}")
    print(f"FX_CONVERTED_CARDS={metrics['fx_converted_cards']}")
    for source_name, count in payload["by_source"].items():
        print(f"ACTIVE_SOURCE_{source_name.upper().replace(' ', '_')}={count}")
    ...
    for key in (
        "deep_overlap_targets",
        "deep_overlap_queries",
        "deep_overlap_added_cards",
        "deep_overlap_errors",
        "exact_discovery_targets",
        "exact_discovery_queries",
        "exact_discovery_added_cards",
        "exact_discovery_matches",
        "exact_discovery_errors",
    ):
